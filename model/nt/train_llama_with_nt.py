...
class LlamaWithNt(nn.Module):
    def __init__(self, config, tokenizer:Optional[str]=None, multimodal_tokenizer:Optional[str]=None):
        super().__init__()
        self.config = config
        nt_config = config.multimodal_model_config
        self.model = Transformer(config)
        self.multimodal_model = NtModel(nt_config)
        ...
        try:
            ...
            if multimodal_tokenizer is None:
                self.multimodal_tokenizer = NtTokenizer(registry.get_path("tokenizer_nt"))
            else:
                self.multimodal_tokenizer = NtTokenizer(multimodal_tokenizer)
        except:
            pass
