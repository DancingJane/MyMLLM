...
# @registry.register_model(["llama1_with_bert_inference", "llama2_with_bert_inference"])
class LlamaWithBert(nn.Module):
    def __init__(self, config, model_ckpt_path:Optional[str]=None, multimodal_model_ckpt_path:Optional[str]=None):
        super().__init__()
        self.config = config
        self.bert_config = config.multimodal_model_config
        self.model = Transformer(config)
        self.multimodal_model = BertModel(self.bert_config)
        ...
        try:
            ...
            self.multimodal_tokenizer = DnaBert2Tokenizer(self.config.multimodal_model_config.tokenizer)
        ...
        if model_ckpt_path and multimodal_model_ckpt_path:
            if model_ckpt_path == multimodal_model_ckpt_path:  # 是同一个ckpt文件
                ...
                if any(k.startswith('model.') for k in full_state_dict.keys()):
                    ...
                    # 处理DNABERT部分
                    bert_state_dict = {k.replace('multimodal_model.', ''): v 
                                     for k, v in full_state_dict.items() 
                                     if k.startswith('multimodal_model.')}
                    self.multimodal_model.load_state_dict(bert_state_dict, strict=False)
...
# @registry.register_model("llama3_with_bert_inference")
...
if __name__ == '__main__':
    model_config = registry.get_model_config_class("llama3_with_bert_large")()
    model_config.tokenizer = '/tos-bjml-ai4agr/lijinzhe/BioModel/Meta-Llama-3-8B-Instruct/original/tokenizer.model'
    model_config.multimodal_model_config.tokenizer = '/tos-bjml-ai4agr/mazhe/model_dir/DNABERT-2-117M/'
    ...
    multimodal_model_ckpt_path = '/tos-bjml-ai4agr/mazhe/model_dir/DNABERT-2-117M/dnabert2.bin'
    model_ckpt_path = '/fs-computility/ai4agr/mazhe/MyMLLM/output_ckpt/multimodal_instruct_tuning_exp_1/final.ckpt'
    multimodal_model_ckpt_path = '/fs-computility/ai4agr/mazhe/MyMLLM/output_ckpt/multimodal_instruct_tuning_exp_1/final.ckpt'

    test_model = LlamaWithBert(model_config, model_ckpt_path, multimodal_model_ckpt_path)
